...
For this problem, I used python.

Note: Unfortunately, by technical issues, R-markdown does not support the matplotlib,
a python library for visualization.
So, in this markdown file, I use R functions to draw figures.
(As a result, this file are mixed with python code and R code.
For pure-python(!) file, visit https://github.com/letsjdosth/intermediateBayesInfer
and see the directory 'HW3'.)

```{r}
library(reticulate) # this library connects R and python
```


# Generate Gumbel type-2 samples using the inverse CDF method
...
```
```{r}
hist(py$gumbel2_smpl_55, breaks=50, freq=FALSE, main="gumbel2_smpl_55")
```


# posterior evaluation on grid

I will make a figure for the log-posterior.
We can easily get the posterior with original scale.
But since logarithm is 1-1 and monotone increasing, information conveyed by a figure is basically same.
To avoid underflow and overflow, I will skip to make a figure with original scale.

Note that, I also skip finding the normalizing constant.
We can find the approximated constant by just adding all values at the proper grid range
that density are meanngfully greater than 0.
In my opinion, however, the constant is useless unless we want to see the true probability
(strictly speaking, actually, it is just 'approximated', not true.)
...
```
```{r}
contour(seq(3,6.9,by=0.1), seq(3,6.9,by=0.1), t(py$log_q_val_vec), nlevels=100,
xlab="alpha", ylab="beta", main="grid evaluation")
```


# Laplace approximation

This class is newton method implementation.
(This is not essential part of our work, so skip it.)
...
To use newton method, set functions about log posterior, its gradient, and its hessian.
Since above class is designed for minimization problem, I make them have negative-value output.
...
After finding the mode (this is convex problem!), I will evaluate negative hessian at the mode.
By inverting the negative hessian, we can get the covariance matrix 
for the multivariate normal distribution from the Laplace approximation.
...
Since started at (5,5), newton method converges by only one step.
(I have checked that the convergence are so fast even if I started far from (5,5).)

Let's draw a contour plot, 
and draw scatterplot of simulated samples from the approximated multivariate normal distribution over the contour plot,
...
log_q_laplace_approxed_val_vec = []
for param in grid_2d:
    log_q_laplace_approxed_val_vec.append(multivariate_normal_log_density(param, posterior_mode, log_posterior_negative_hessian))
log_q_laplace_approxed_val_vec = np.array(log_q_laplace_approxed_val_vec).reshape((40,40)).transpose()

# plt.contour(grid_1d_mesh_X, grid_1d_mesh_Y,log_q_laplace_approxed_val_vec, levels=100) 
...
```
```{r}
plot(py$laplace_approx_sample, xlim=c(3,7), ylim=c(3,7),
xlab="alpha", ylab="beta", main="Laplace approximation", pch=19, cex=0.5, col="blue")
contour(seq(3,6.9,by=0.1), seq(3,6.9,by=0.1), t(py$log_q_laplace_approxed_val_vec), nlevels=100, add=TRUE)
abline(v=py$posterior_mode[1], h=py$posterior_mode[2], col="red")
```


To compare, let me repeat the two plots in the above.
...
Since our posterior looks good to approximate by multivariate normal distribution,
(one mode, quite symmetric with respect to the mode, etc)
Laplace approximation has worked well.
